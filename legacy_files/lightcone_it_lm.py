...
#Relaxation scheme with pure alms by pysh
...
class Lightcone:
        ...
        for field in self.sols:
            for step in reversed(range(self.ntau_f + 1, self.Ntau)):
                ...
                if field == 'Phi':
                    ...
                    if(self.grid=='DH2'):
                        real_err = pysh.expand.MakeGridDH(rhs - lhs, sampling = 2, extend = False)
    ...
    def init_from_slice(self, z_i_in, r_max_in, Phi_i_in, Pi_i_in, Params, \
        ...
        self.Phi = npy.array( [pysh.expand.SHExpandDH( \
                                                       self.sols['Phi'][n], sampling = 2) \
                               for n in range(self.Ntau+1)])
        self.Pi = npy.array( [pysh.expand.SHExpandDH( \
                                                       self.sols['Pi'][n], sampling = 2) \
                               for n in range(self.Ntau+1)])
        self.delta = npy.array( [pysh.expand.SHExpandDH( \
                                                      self.matter['delta'][n], sampling = 2) \
                              for n in range(self.Ntau+1)])
        self.vw = npy.array( [pysh.expand.SHExpandDH( \
                                                         self.matter['vw'][n], sampling = 2) \
                                 for n in range(self.Ntau+1)])
    ...
    def build_lcmetric(self):
        ...
        for n in range(self.ntau_f+1, self.Ntau):
            for field in self.sols:
                self.sols[field][n] = pysh.expand.MakeGridDH( \
                    eval('self.'+field)[n], sampling = 2, extend = False)
