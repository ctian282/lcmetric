...
from libc.math cimport sin, cos, acos, exp, sqrt, fabs, M_PI, floor
...
cdef inline double to_tau(int ntau, int Ntau, double tau_i):
     return tau_i * ntau / Ntau 

cpdef dPi_dt(int ntau, int Ntau, double tau_i, double Omega_m, \
             double Hubble_0, double Hubble, double Hubble_dt, double a, \
             npy.ndarray[double, ndim = 1] Phi, \
             npy.ndarray[double, ndim = 1] Pi, \
             npy.ndarray[double, ndim = 1] Omega, \
             npy.ndarray[double, ndim = 1] delta, \
             npy.ndarray[double, ndim = 1] vw, \
             npy.ndarray[double, ndim = 1] angle_lap):
     return  (2 / to_tau(ntau, Ntau, tau_i) - 4*Hubble) * Pi \
            - 3 * Hubble * Omega \
            - 3 * Hubble**2 * Phi \
            + angle_lap \
            - 1.5 * Hubble_0**2 * Omega_m * delta \
            / a\
...
class Lightcone:
    ...
    def dPi_dt(self, ntau):
        return (2 / to_tau(ntau, self.Ntau, self.tau_i) - 4*self.metric_f['Hubble'][ntau]) * self.sols['Pi'][ntau] \
    ...
    def dOmega_dt(self, ntau):
        return (2 / to_tau(ntau, self.Ntau, self.tau_i) + 2*self.metric_f['Hubble'][ntau]) * self.sols['Pi'][ntau] \
    ...
    def est_errors(self):
        ...
        #self.test = dPi_dt(240, self.sols['Phi'][240])
        for field in self.sols:
            # if(field == 'Pi'):
            ...
            #         - dPi_dt(step, self.Ntau, self.tau_i, self.Omega_m, \
            #                  self.Hubble_0, self.metric_f['Hubble'][step], \
            #                  self.metric_f['Hubble_dt'][step], self.metric_f['a'][step], \
            #                  self.sols['Phi'][step], self.sols['Pi'][step],\
            #                  self.sols['Omega'][step], self.matter['delta'][step], \
            #                  self.matter['vw'][step], self.angle_lap \
            #         )  \
            #         ).max(), errs[field]])
    ...
    def est_angle_lap(self, ntau):
        ...
        self.angle_lap = hp.alm2map(alm, self.nside, pixwin=False) / to_tau(ntau, self.Ntau, self.tau_i)**2
