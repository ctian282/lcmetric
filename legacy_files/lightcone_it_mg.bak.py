...
class Lightcone:
    ...
    def  __init__(self,  nlat_in, lmax_in = None, \
                  ...
                  angle_lap_type = 'SPH', depth = 3, n_vcycles = 2, npre = 10, npost = 10):
    ...
    def est_errors(self, d):
        ...
        for field in self.sols:
            if field == 'Phi':
                print('Max error for field '+field+' is '+str(err))
    ...
    def update_rhs(self, d):
        for step in  range(self.h_size[d] - 2, 0, -1):
            self.est_angle_lap(step, d)
            self.rhs_hier[d] += (self.Phi_hier[d][step + 1] + self.Phi_hier[d][step - 1] \
                                 - 2.0 * self.Phi_hier[d][step ])/ (self.dtau_hier[d])**2\
    ...
    def FAS(self):
        ...
        while(n_vcycles >0):
            ...
            self.est_errors(0)
            for d in range(self.depth):
                self.relax(d, self.npre)
                print('    At level '+str(d)+', after '+str(self.npre)+' relaxation')
                self.est_errors(d)
                if(d < self.depth - 1):
                    self.generate_rl(d)
            ...
            for d in range(self.depth - 1, -1, -1):
                self.relax(d, self.npost)
                print('    At level '+str(d)+', after '+str(self.npost)+' relaxation')
                self.est_errors(d)
                if(d > 0):
